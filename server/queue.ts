...
async function processAnalysis(job: Job<{ analysisId: number; repositoryId: number; pullRequestNumber: number; headSha: string }>) {
  ...
  try {
    ...
    const result = await runAIReview({ diff, files: changedFiles, nearbyTests, manifests, additions: pullRequest.additions, deletions: pullRequest.deletions, workspaceId: repository.workspaceId });
